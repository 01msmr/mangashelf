...
### Texte zentralisiert (i18n)

- **Hardcodierte JS/HTML-Strings nach `lang.json` geholt:** „Logout" (`nav.js` → `nav.logout`), Promote-Tooltips „Make admin"/„Remove admin rights" (→ `adminUsers.makeAdmin/removeAdmin`), die `data-tip`-Tooltips „Numpad"/„Lookup"/„Camera" (→ `common.numpad`, `addBook.lookup/camera`, per `Lang.t` gesetzt) und alle `'Failed'`/`'Error'`-Fallbacks (→ `common.error`). Neue Keys in EN/DE/Schwäbisch. Verifiziert (DE): „Abmelden", „Zum Admin machen", „Ziffernblock", „Nachschlagen".
